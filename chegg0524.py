...
class Chegg:
    ...
    def Regression_c(self):
        ...
        for i,j in zip(self.__x,self.__y):
            ...
            s1xy=f'({(i-mx):.4f}*{(j-my):.4f})' # brick string 1
            s1xx=f'({(i-mx):.4f})^2'            # c=s1
            s1yy=f'({(j-my):.4f})^2'

            s2xy=f'({((i-mx)*(j-my)):.4f})' # brick sring 2
            s2xx=f'({((i-mx)**2):.4f})'     # c1=s2
            s2yy=f'({((j-my)**2):.4f})'
            ...
            str_xy_list.append(sxy)
            str_xx+=sxx
            str_xx_list.append(sxx)
            str_yy+=syy
            str_yy_list.append(syy)

            str1_xy+=s1xy                 #cal_xy=str1_xy
            str1_xy_list.append(s1xy)
            str1_xx+=s1xx
            str1_xx_list.append(s1xx)
            str1_yy+=s1yy
            str1_yy_list.append(s1yy)

            str2_xy+=s2xy                #cal1_xy=str2_xy
            str2_xy_list.append(s2xy)
            str2_xx+=s2xx
            str2_xx_list.append(s2xx)
            str2_yy+=s2yy
            str2_yy_list.append(s2yy)

            #cal_xy+=calxy                # new
            cal_xy_list.append(calxy)
            #cal_xx+=calxx
            cal_xx_list.append(calxx)
            #cal_yy+=calyy
            cal_yy_list.append(calyy)

        self.__display(self.__Markdown("# Regression"))
        print("\n................................")
        
        #SLOPE
        numerator1=self.short(str_xy_list)
        numerator2=self.short(str1_xy_list)
        numerator3=self.short(str2_xy_list)

        denominator1=self.short(str_xx_list)
        denominator2=self.short(str1_xx_list)
        denominator3=self.short(str2_xx_list)


        beta_1=sum(cal_xy_list)/sum(cal_xx_list)
        
        self.__display(self.__Markdown("## Slope :\n"))
        self.__display(self.__Math(r'$Slope\;(\beta_1) = \frac{\sum_{i=1}^{n} (x_i - \bar{x})(y_i - \bar{y})}{\sum_{i=1}^{n} (x_i - \bar{x})^2}$'))
        print("\nFormula :\nbeta_1=(sum(x-barx)(y-bary))/(sum(x-barx)^2)\n\n................................")
        self.__display(self.__Markdown("### Syntax :"))
        
        print("STEP 00 :\nbeta_1=(sum(x-barx)(y-bary))/(sum(x-barx)^2)")
        print(f"\nSTEP 01 :\n({numerator1})/({denominator1})")
        print(f"\nSTEP 02 :\n({numerator2})/({denominator2})")
        print(f"\nSTEP 03 :\n({numerator3})/({denominator3})")
        print(f"\nSTEP 04 :\n({round(sum(cal_xy_list),4)})/({round(sum(cal_xx_list),4)})")
        print(f"\nSTEP 05 :\n{beta_1}")
        print("\n................................")
        
        
        # INTERCEPT
        self.__display(self.__Markdown("## Intercept :\n"))
        self.__display(self.__Math(r'$Intercept\;(\beta_0) = \bar{y}-\beta_1\bar{x}$'))     # beta_0=bary-beta_1barx
        print("\nFormula :\nbeta_0=bary-beta_1barx\n\n................................")
        self.__display(self.__Markdown("### Syntax :"))
        print(f"STEP 01 :\nbeta_0=bary-beta_1barx")
        print(f"\nSTEP 02 :\n{my}-[{beta_1}*{mx}]")
        print(f"\nSTEP 03 :\n{my}-({beta_1*mx})")
        ...
        x1=self.__np.array(self.__x)
        y1=self.__np.array(self.__y)
        X = x1.reshape((-1, 1))
